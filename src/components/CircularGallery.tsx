...
function autoBind(instance: Record<string, unknown>): void {
  ...
  Object.getOwnPropertyNames(proto).forEach((key) => {
    if (key !== "constructor" && typeof instance[key] === "function") {
      (instance as Record<string, unknown>)[key] = (instance[key] as () => unknown).bind(instance);
    }
  });
}
...
class Media {
  ...
  onResize({ screen, viewport }: ResizeParams = {}) {
...
class App {
  ...
  constructor(container: HTMLElement, { items, bend = 3, textColor = "#fff", borderRadius = 0, font = "bold 30px Figtree", scrollSpeed = 2, scrollEase = 0.05 }: AppConstructorParams) {
  ...
  createMedias(items: Array<{ image: string; text: string }>, bend: number, textColor: string, borderRadius: number, font: string) {
    const defaultItems = [
      { image: "https://picsum.photos/800/600?1", text: "Event 1" },
      { image: "https://picsum.photos/800/600?2", text: "Event 2" },
      { image: "https://picsum.photos/800/600?3", text: "Event 3" },
    ];
    const galleryItems = items && items.length ? items : defaultItems;
    const loopItems = galleryItems.concat(galleryItems);
    this.medias = loopItems.map(
      (data, index) =>
        new Media({
...
export default function CircularGallery({
  ...
  return <div className="w-full h-full overflow-hidden bg-black" ref={containerRef} />;
}
