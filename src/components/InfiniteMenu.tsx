...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  {activeItem && (
    <>
      {/* Title - Left Center (smaller, cyan, outside image area) */}
      <h2
        className={`absolute left-8 md:left-12 top-1/2 -translate-y-1/2 
        text-cyan-400 font-bold text-2xl md:text-3xl lg:text-5xl 
        select-none z-20
        transition-all duration-500 ease-[cubic-bezier(0.25,0.1,0.25,1.0)]
        ${
          isMoving
            ? 'opacity-0 pointer-events-none duration-[100ms] translate-x-[-40px]'
      ...
      <div
        ...
        className={`absolute left-1/2 z-30 w-[60px] h-[60px] grid place-items-center 
        bg-[#13a2d5] border-[5px] border-black rounded-full cursor-pointer 
